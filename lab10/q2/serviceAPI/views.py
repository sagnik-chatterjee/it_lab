...
def index(request):
    return render(request, "serviceAPI/index.html")


# URL: serviceAPI/driver/register
...
# URL: serviceAPI/booking/<commuter_id>/<driver_id>
...
def BookCab(request, commuter_id, driver_id):
    ...
    try:
        ...
        return Response(
            {"error": "Driver with this ID {} does not exist".format(driver_id)}
        )
    if (
        driver.status == "AV"
    ):  # filters out those drivers who are currently booked and offline
        driver.status = "BK"
        driver.save()
...
# URL: /serviceAPI/driver/available
@api_view(["GET"])
...
# URL: /serviceAPI/user/<commuter_id>/history
@api_view(["GET"])
def TripHistoryUser(request, commuter_id):
    if request.method == "GET":
        try:
            ...
            return Response(
                {"error": "Driver with ID {} is unavailable".format(commuter_id)}
            )
...
# URL: /serviceAPI/driver/<driver_id>/history
@api_view(["GET"])
def TripHistoryDriver(request, driver_id):
    if request.method == "GET":
        try:
            ...
            return Response(
                {"error": "Driver with ID {} is unavailable".format(driver_id)}
            )
...
# URL: /serviceAPI/update/<driver_id>
# POST Params: [status]
...
def ChangeStatus(request, driver_id):
    if request.method == "POST":
        try:
            ...
            return Response(
                {"error": "Driver with ID {} does not exist".format(driver_id)}
            )
...
# URL: /serviceAPI/user/location/commuter_id
...
def UserLocation(request, commuter_id):
    try:
        ...
        return Response(
            {"error": "commuter with ID {} does not exist".format(commuter_id)}
        )
    if request.method == "GET":
